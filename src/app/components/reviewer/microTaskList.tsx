...
const MicroTaskList: React.FC<MicroTaskListProps> = ({
  ...
  const rejectionReasons =
    rejectionReasonsData && Array.isArray(rejectionReasonsData.data)
      ...
      : [];
  // Approve dialog offers only positive-sentiment annotations; the negatives are
  // covered by the separate rejection-reason list used in the Reject flow.
  const annotations =
    dynamicResponsedataAnnotation &&
    Array.isArray(dynamicResponsedataAnnotation.data)
      ? dynamicResponsedataAnnotation.data.filter(
  ...
  const microTaskColumns: ColumnDef<ReviewerDatset>[] = [
    ...
    },
    {
      ...
      header: t("ageHeader"),
      cell: ({ row }) => {
        if (row.original.status?.toLowerCase() !== "pending" || !row.original.created_date) {
          return <span className="text-gray-400">—</span>;
        }
        const hours =
          (Date.now() - new Date(row.original.created_date).getTime()) /
          (1000 * 60 * 60);
        const label =
          hours < 24
            ? `${Math.round(hours * 10) / 10}h`
            : `${Math.floor(hours / 24)}d ${Math.round(hours % 24)}h`;
        // Flags a queue item that's been sitting long enough to be worth a
        // second look -- 24h amber, 72h red -- the exact "how long has this
        // been waiting" signal that was missing from every review surface.
        const colorClass =
          hours >= 72
                ...
                <div className="sticky top-0 z-10 p-10 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setReviewedItems({});
                      onInnerDialogOpenChange?.(false);
                      // Don't reset to page 1 here -- a reviewer exiting
                      // mid-queue (e.g. from page 3) should land back on the
                      // page they were on, not lose their place. Page only
                      // resets when the status filter itself changes (see
                      // the effect above).
                      queryClient.invalidateQueries({
                        queryKey: ["taskMicroTasksResultReviewers"],
                      });
                      ...
                      !isRefetching ? (
                        <>
                          <div className="mb-4 border border-gray-100 rounded-lg px-6 py-3">
                            ...
                            {microtasks[currentRowIndex].contributor && (
                              <div className="flex flex-wrap items-center gap-2 px-4 mb-3 text-sm">
                                ...
                                {typeof microtasks[currentRowIndex].contributor
                                  .score === "number" && (
                                  <span className="px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 font-medium">
                                    {t("contributorScore")}:{" "}
                                    {microtasks[currentRowIndex].contributor.score}
                                  </span>
                                )}
                              </div>
                            )}
                            {microtasks[currentRowIndex].rejectionReasons
                              ?.length > 0 && (
                              <div className="mx-4 mb-4 px-4 py-3 bg-red-50 border border-red-100 rounded-lg">
                                <p className="text-sm font-semibold text-red-700 mb-1">
                                  {t("previouslyRejectedFor")}
                                </p>
